...
are inferred from the source material. Do **not** invent business rules — but
where the source is silent on something a developer obviously needs (folder
layout, indexes, middleware, role table, sample code, state machine, etc.),
you may add it and clearly mark it as `[Extended]`.
...
7. Mark every item that is **not literally present in the source** as
   `[Extended]` at the end of the line / row / bullet. If a whole section is
   extended, put `[Extended]` next to the heading.
8. Where the source contradicts itself or is missing a number/rule, add a small
   inline note: `> ⚠ Source gap: <what is missing>. Filled with sensible default
   marked [Extended].`
9. Prefer **plain English first, then the technical detail**. A BA must be able
   to read the first paragraph of every section and understand the intent.
10. **No emojis** anywhere except the warning glyph `⚠` for source gaps and the
...
> SRS v<x> Module <id>". Then a one-paragraph **Legend** explaining that items
> from the source are plain and additions are tagged [Extended].
...
- Cite the source clause inline where applicable, e.g. `(SRS §3.6)` or
  `(BRD-FR-12)`. If the citation is invented, mark `[Extended]`.
...
|---|----------------------------------|------------------|---------|
| 1 | High-level architecture           | flowchart TB     | §3      |
| 2 | Tenant / deployment isolation     | flowchart LR     | §4      |
| 3 | Pages & roles map                 | flowchart TB     | §6      |
| 4 | Generic login flow                | sequenceDiagram  | §8      |
| 5 | Service folder layout             | flowchart TB     | §9      |
| 6 | Database ER                       | erDiagram        | §10     |
| 7+| One sequence diagram per use case | sequenceDiagram  | §12     |
| n | State machine                     | stateDiagram-v2  | §13     |

---

## 5. Tagging Convention — `[Extended]`

Anything **not literally written in the supplied source** is `[Extended]`.
Apply at the smallest unit possible:

- Heading-level: `### 4.2 Tenant binding for biometric devices [Extended]`
- Row-level (in tables): add a column `Source` with values `SRS` / `[Extended]`
  OR append `[Extended]` to the cell.
- Bullet-level: `- Redis cache for roster lookup [Extended]`
- API/endpoint: `### 11.7 POST /api/v1/.../leave-application [Extended]`
- Field/column: `(added) tenant_id [Extended]`

If you find a **loophole, ambiguity, or missing detail** in the source, add a
short note in section 19 (Open Questions / Source Gaps) **and** mark the place
where you filled it with `[Extended]`.

> Rule of thumb: if a Business Analyst removed every `[Extended]` line, the
> remainder must be a faithful read of the original source.
...
- **Concrete numbers.** If the source says "fast", quote what fast means
  (latency target). If the source is silent, give a default and tag `[Extended]`.
- Never apologise, never hedge ("might", "could maybe"). State the design and
  tag uncertainty as `[Extended]` or as a `⚠ Source gap` note.

## 6a. Depth Contract — exhaustive, not summary
...
- **§3 Architecture.** Walk the request flow step-by-step. For each component,
...
- **§4 Multi-tenant / deployment.** Cover URL-to-tenant resolution, JWT shape,
  ...
  attack prevention.
- **§6 Pages / screens.** For *every* screen, document: who can reach it,
...
- **§7 Roles & access.** Build a full matrix: every role × every page × every
  ...
  (gateway, service, DB).
- **§8 Login / entry flow.** One numbered flow per role. Include MFA, SSO,
  device-binding, OTP fallback, account-lock policy, session length.
- **§9 Service folder & file structure.** Print the actual tree. For every
  file, one line on what lives there. Include `migrations/`, `cron/`, `events/`,
  `auth/`, `tenant/`, `repo/`, `api/`, `domain/`, configs, Dockerfile.
- **§10 Database design.** Every table, every column, type, nullability,
...
- **§11 API list.** *Every* endpoint. For each: method, path, purpose, who
...
- **§12 Core flows.** *Every* use case from the source plus every `[Extended]`
...
- **§13 State machine.** Every state, every transition, who can perform it,
  when, the side effects (notifications, audit, KPI updates), and the
  rollback rules.
- **§14 Sample code.** Provide real, compileable snippets for: the auth
...
- **§15 Reports.** Every report: who consumes it, what columns, what
  filters, frequency, format (PDF/Excel/CSV), and the SQL/aggregation outline.
- **§16 Security, audit & data safety.** Cover transport, at-rest, secrets,
  audit log shape, immutable retention, MFA matrix, data-residency, DPDP /
  GDPR / FERPA / HIPAA mapping if relevant.
- **§17 Performance & scale.** Per-endpoint p50/p95/p99 targets, throughput
...
- **§18 End-to-end scenarios.** *Multiple* scenarios — at minimum: golden
...
- **§19 Open questions / source gaps.** Every ambiguity surfaced.
- **§20 Summary checklist.** Split into Source must-haves vs `[Extended]`,
  with a `☐` for each, mapped back to the FR or section that proves it.
...
- Mark snippets as `[Extended]` if the source did not specify them.

---

## 8. Tables — Required Shape (A4-responsive)
...
| Source column | Required for any table that mixes source + Extended rows. Values: `SRS` / `BRD` / `[Extended]` |
...
## 10. Final Self-Check (run before responding)
...
4. ☐ Every `[Extended]` item is justified (not in source).
5. ☐ Every cited clause (`SRS §x.y`) actually exists in the source. If invented,
     change to `[Extended]`.
6. ☐ No raw HTML other than `<br/>` inside Mermaid and tables.
7. ☐ Section 19 lists all source gaps.
8. ☐ **Every table has ≤ 6 columns** and no cell holds a long URL together
     with another long string. Long endpoint paths are inside backticks.
9. ☐ **Every section in §3a is covered at depth.** No section is a one-liner;
     no table is a placeholder; no flow is summarised away.
10. ☐ Every actor / role / use-case / FR / NFR / table / report / KPI / error
     code / threshold mentioned in the source has a corresponding home in this
     document.
11. ☐ Final line is `*— End of Document —*`.

If any check fails, fix silently and re-emit. Do not narrate the self-check.

---

## 11. Invocation Pattern (for the user, not the AI)
...
> with a sensible default and tag it `[Extended]`, and list the gap in §19."
